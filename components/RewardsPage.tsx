 ...
 * Học sinh có thể xem items đã/chưa unlock, và trang bị frame.
 */
...
const RewardsPage: React.FC<RewardsPageProps> = ({ user, onEquipFrame, onBack }) => {
  ...
  const totalItems = WEEKLY_FRAMES.length * 3; // 24
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-500 pb-20 px-4">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-[32px] p-4 sm:p-8 flex items-center gap-4 sm:gap-8">
        ...
        {/* Weekly XP Progress toward next milestone */}
        {currentWeek && (
            ...
            <div
              ...
              className={`relative bg-slate-900 border-2 rounded-[28px] p-6 transition-all ${
                isEquipped
                  ...
                  : isComplete
                  ? 'border-slate-700 hover:border-opacity-60'
            ...
            >
              ...
              <div className="space-y-2 mb-4">
                {frame.items.map((item, idx) => {
                  const isUnlocked = (user.unlockedFrames || []).includes(item.id);
                  return (
                    <div
                      key={item.id}
                      className={`flex items-center gap-3 p-3 rounded-xl ${
                        isUnlocked ? 'bg-slate-800' : 'bg-slate-950/50'
                      }`}
                    >
      ...
      <p className="text-center text-xs text-slate-600 pb-4">
        Đạt {WEEKLY_FRAMES[0].items[0].xpRequired.toLocaleString()} / {WEEKLY_FRAMES[0].items[1].xpRequired.toLocaleString()} / {WEEKLY_FRAMES[0].items[2].xpRequired.toLocaleString()} XP mỗi tuần để mở khóa 3 items của tuần đó
      </p>
    </div>
  );
};
