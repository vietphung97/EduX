 ...
 * Học sinh có thể download / chụp màn hình.
 */
...
const CertificatePage: React.FC<CertificatePageProps> = ({ user, onBack }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-20 px-4 animate-in fade-in duration-500">
      ...
      >
        <div className="bg-[#0f172a] rounded-[28px] p-10 sm:p-14 relative overflow-hidden">
          {/* Background decoration */}
          <div className="absolute top-0 left-0 w-full h-full opacity-5 pointer-events-none select-none" style={{
          ...
          <div className="relative z-10 text-center space-y-8">
            ...
            <p className="text-slate-400 text-sm font-medium max-w-xs mx-auto">
              Chứng nhận học sinh đã hoàn thành chương trình thi đấu Tiếng Anh hè
            </p>
            ...
            {(user.unlockedFrames?.length || 0) > 0 && (
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-2">
                  Phần thưởng đã nhận ({user.unlockedFrames!.length} items)
                </p>
                <div className="flex justify-center gap-2 flex-wrap">
                  {user.unlockedFrames!.slice(0, 12).map(itemId => {
                    // find emoji for this item (simplified)
                    return (
                      <span key={itemId} className="text-lg" title={itemId}>🏅</span>
                    );
                  })}
                </div>
              </div>
            ...
            {/* Footer */}
            <div className="pt-6 border-t border-slate-800/50 flex justify-between items-center text-xs">
